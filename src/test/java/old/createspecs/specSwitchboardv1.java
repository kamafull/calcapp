...
public class specSwitchboardv1 {
    ...
    public void createspec(typicalSb sw) throws SQLException {
        ...
        try (Connection con = DriverManager.getConnection(url, user, password); Statement stat = con.createStatement()) {
            ...
            sql = "SELECT * FROM circuit_breakers.power_meters WHERE need_net = " + sw.getPmeter_need() + " and manufacturer = 84;";
            rs = stat.executeQuery(sql);
            if (rs.next()) {
                row.createCell(0).setCellValue(rs.getString("description"));
                row.createCell(1).setCellValue(mnfback.mnfname(84));
                row.createCell(2).setCellValue(rs.getString("id_manufacturer"));
